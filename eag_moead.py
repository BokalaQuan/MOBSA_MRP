...
class ExternalArchiveGuidedMOEAD(MOEA):
        ...
        if p_sel <= p_list[0]:
            ...
            for i in range(1, POPULATION_SIZE):
                if p_sel > p_list[i - 1] and p_sel <= p_list[i]:
                    select_sub = self.current_population[i]
                    break

        return select_sub

    def reproduction(self, sub):
        index1 = 0
        index2 = 0
        while index1 == index2:
            index1 = random.randint(0, len(sub.index_neighbor)-1)
            index2 = random.randint(0, len(sub.index_neighbor)-1)

        ind1 = self.current_population[sub.index_neighbor[index1]].solution.copy()
        ind2 = self.current_population[sub.index_neighbor[index2]].solution.copy()
        ...
        sub.set_of_solutions.append(ind_new)

        return ind_new

    def update_neighbor_solution(self, sub):
        for i in sub.index_neighbor:
            sub_select = self.current_population[i]

            for sol in sub.set_of_solutions:
                fit1 = sub.cal_fit(sol, sub_select.weight_vector, 'WS')
                fit2 = sub.cal_fit(sub_select.solution, sub_select.weight_vector, 'WS')

                if fit1 < fit2:
                    sub_select.solution = sol.copy()
    ...
    def run(self):
        ...
        while gen < MAX_NUMBER_FUNCTION_EVAL:
            if gen < self.L:
                select_subpop = self.current_population
            else:
                P_list = self.evaluate_population(gen=gen)
                select_subpop = [self.select_subproblem(P_list) for i in range(POPULATION_SIZE)]

            new_solutions = [self.reproduction(sub) for sub in select_subpop]

            for sub in set(select_subpop):
